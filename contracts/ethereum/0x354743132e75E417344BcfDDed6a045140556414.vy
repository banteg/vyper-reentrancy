# @version 0.2.16
"""
@title APWine Fee Distribution
@author APWine Protocol
@license MIT
"""
...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the veAPW balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 veAPW balance
    """
...
def checkpoint_total_supply():
    """
    @notice Update the veAPW total supply checkpoint
...
def claim(_addr: address = msg.sender) -> uint256:
    ...
    @dev Each call to claim look at a maximum of 50 user veAPW points.
         For accounts with many veAPW related actions, this function
