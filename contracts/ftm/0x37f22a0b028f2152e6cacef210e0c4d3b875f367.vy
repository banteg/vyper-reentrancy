...
interface CurvePlainPool:
    def initialize(
        ...
        _rate_multipliers: uint256[MAX_PLAIN_COINS],
        _A: uint256,
        _fee: uint256,
    ): nonpayable
...
MAX_PLAIN_COINS: constant(int128) = 6  # max coins in a plain pool
...
def get_balances(_pool: address) -> uint256[MAX_PLAIN_COINS]:
    ...
    """
    if self.pool_data[_pool].base_pool != ZERO_ADDRESS:
        return [CurvePool(_pool).balances(0), CurvePool(_pool).balances(1), 0, 0, 0, 0]
...
def deploy_plain_pool(
    ...
    """
    # fee must be between 0.001% and 1%
    assert _fee >= 100000 and _fee <= 100000000, "Invalid fee"
...
def deploy_metapool(
    ...
    self.pool_data[pool].decimals = [decimals, 0, 0, 0, 0, 0]
