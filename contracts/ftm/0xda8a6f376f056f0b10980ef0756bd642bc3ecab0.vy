...
@view
@internal
def rate_multipliers() -> uint256[N_COINS]:
    multipliers: uint256[N_COINS] = empty(uint256[N_COINS])

    for i in range(N_COINS):
        coin: address = self.coins[i]
        multipliers[i] = Pool(coin).get_virtual_price()
...
def ramp_A(_future_A: uint256, _future_time: uint256):
    assert msg.sender == self.factory  # dev: only owner
