...
MAX_INPUT_SIZE: constant(uint256) = 1000
...
def _exchange(
    ...
    if _to == ETH_ADDRESS:
        ...
        raw_call(_sender, b"", value=received_amount)
    else:
    ...
    log TokenExchange(_sender, _sender, _pool, _from, _to, _amount, received_amount)
...
def exchange_with_best_rate(
    ...
    return self._exchange(registry, best_pool, _from, _to, _amount, _expected, msg.sender)
...
def exchange(
    ...
    return self._exchange(self.registry, _pool, _from, _to, _amount, _expected, msg.sender)


@view
@external
def get_best_rate(
    _from: address, _to: address, _amount: uint256) -> (address, uint256):
    """
...
@view
@external
def get_rates(_from: address, _to: address, _amounts: uint256[MAX_INPUT_SIZE]) -> uint256[MAX_INPUT_SIZE]:
    registry: address = self.registry
    rates: uint256[MAX_INPUT_SIZE] = empty(uint256[MAX_INPUT_SIZE])
    best_pool: address = ZERO_ADDRESS
    ...
    for i in range(65536):
        ...
        if pool == ZERO_ADDRESS:
            break
        dy: uint256 = self._get_exchange_amount(registry, pool, _from, _to, _amounts[0])
        if dy > max_dy:
            best_pool = pool
            max_dy = dy

    if best_pool != ZERO_ADDRESS:
        for i in range(MAX_INPUT_SIZE):
            rates[i] = self._get_exchange_amount(registry, best_pool, _from, _to, _amounts[i])
